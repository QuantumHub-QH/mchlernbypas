...
GOOGLE_CLIENT_ID = os.environ.get("GOOGLE_CLIENT_ID", "") # Diisi di Railway nanti
...
def upload():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            increment_usage(identifier) # ✅ INCREMENT LIMIT KARENA SUKSES
            return jsonify({"success": True, "asset_id": asset_id, "title": title, "blocked": False})
        else:
            blocked = resp.status_code in (403, 401)
            # Jika gagal upload karena diblokir roblox, kita anggap kuota terpotong atau tidak? 
            # Kita potong saja kuotanya agar tidak dispam
        ...
        return jsonify({"success": False, "error": str(e), "asset_id": "-", "blocked": False})
    finally:
        for fp in [temp_out]:
